...
contract SheeshToken is Context, IBEP20, Ownable {
  ...
  uint256 private _maxTx = 0.1 * 10 ** 6 * 10 ** _decimals;
  // liquidity threshold
  uint256 private _thresholdToAddLiquidity = 0.0001 * 10 ** 6 * 10 ** _decimals;
  ...
  string private _name = "Sheesh Token";
  ...
  */
  ...
  /*
  * Sets the liquidity fee for the token.
  */
  function setFee(uint256 fee) public onlyOwner returns(bool) {
      require(fee >= 0, "Fee must be greater than or equal to 0.");
      _pFee = _fee;
      _fee = fee;
      return true;
  }
  ...
  /*
  * Sets the reflection tax for the token.
  */
  function setTax(uint256 tax) public onlyOwner returns(bool) {
      require(tax >= 0, "Tax must be greater than or equal to 0.");
      _pTax = _tax;
      _tax = tax;
      return true;
  }
